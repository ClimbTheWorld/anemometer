...
//###### write to log2.txt ##########################################################################
  printf("OPEN log2.txt %d\n", f_open(&File1, "log2.txt", FA_CREATE_ALWAYS | FA_WRITE | FA_READ));
  //f_printf(&File1, "%6d", -200);
  printf("WRITE %d\n", f_write(&File1, "zalue", strlen("zalue"), &bw));
  printf("CLOSE %d\n", f_close(&File1));
//##### log2.txt closed #############################################################################
...
//###### append to log2.txt ##########################################################################
  printf("OPEN log2.txt %d\n", f_open(&File1, "log2.txt", FA_OPEN_EXISTING | FA_WRITE | FA_READ));
  //f_printf(&File1, "%6d", -200);
  /* Move to end of the file to append data */
  printf("GOTO EOF %d\n", f_lseek(&File1, File1.fsize)); //LUK param2 von f_lseek war File1->fsize --> error 2011-01-08
  printf("WRITE %d\n", f_write(&File1, "@ @lukasi", strlen("@ @lukasi"), &bw));
  printf("CLOSE %d\n", f_close(&File1));
//##### append to log2.txt closed #############################################################################

//###### write to log2_jfelk-fkeifsdfkjewhnfiu.txt ##########################################################################
  printf("OPEN log_lang.txt %d\n", f_open(&File1, "log2_jfelkffkeifsdfkjewhnfiu.txt", FA_CREATE_ALWAYS | FA_WRITE | FA_READ));
  //f_printf(&File1, "%6d", -200);
  printf("WRITE %d\n", f_write(&File1, "zalue", strlen("zalue"), &bw));
  printf("CLOSE %d\n", f_close(&File1));
//##### log2.txt closed #############################################################################

//###### write to log8char.txt ##########################################################################
  printf("OPEN log8char.txt %d\n", f_open(&File1, "log8char.txt", FA_CREATE_ALWAYS | FA_WRITE | FA_READ));
  //f_printf(&File1, "%6d", -200);
  printf("WRITE %d\n", f_write(&File1, "zalue", strlen("zalue"), &bw));
  printf("CLOSE %d\n", f_close(&File1));
//##### log2.txt closed #############################################################################
...
//###### write to log2_jfelk-fkeifsdfkjewhnfiu.txt ##########################################################################
  printf("OPEN log-lang.txt %d\n", f_open(&File1, "log2-jfelkffkeifsdfkjewhnfiu.txt", FA_CREATE_ALWAYS | FA_WRITE | FA_READ));
  //f_printf(&File1, "%6d", -200);
  printf("WRITE %d\n", f_write(&File1, "zalue", strlen("zalue"), &bw));
  printf("CLOSE %d\n", f_close(&File1));
//##### log2.txt closed #############################################################################

//###### write to log9chars.txt ##########################################################################
  printf("OPEN log9char.txt %d\n", f_open(&File1, "log9chars.txt", FA_CREATE_ALWAYS | FA_WRITE | FA_READ));
  //f_printf(&File1, "%6d", -200);
  printf("WRITE %d\n", f_write(&File1, "zalue", strlen("zalue"), &bw));
  printf("CLOSE %d\n", f_close(&File1));
//##### log2.txt closed #############################################################################
...
//LUK  uipStart (CFG_AUTO_DHCP_VALUE);
#endif
...
{
  return xTaskCreate (vMonitorTask,  (const signed portCHAR * const) "Monitor", 1024/*3000*/, NULL, (tskIDLE_PRIORITY + 3), &taskHandles [TASKHANDLE_MONITOR]);
}
