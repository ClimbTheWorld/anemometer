...
//LUK
// write protect: GPIO_IO_P13 --> GPIO_IO_P13 --> P1.24
// card present:  GPIO_IO_P12 --> GPIO_IO_P12 --> P1.25  
#define WRPROTECT         GPIO_IO_P24
#define CARDPRESENT       GPIO_IO_P25
...
{
  return (GPIO1_FIOPIN & (volatile)CARDPRESENT) ? 0 : 1;
  //return (GPIO1_IOPIN & (volatile)CARDPRESENT) ? 0 : 1;
}
...
inline int sspHardwareWriteProtected (void)
{
  ...
  //return (GPIO1_FIOPIN & (volatile)WRPROTECT) ? 1 : 0;
  return (GPIO1_IOPIN & (volatile)WRPROTECT) ? 1 : 0;
}
...
void sspInit (void)
{
  ...
  //
  //  //LUK_P0.20 as chip select (output), P0.12 as card present (input), and P0.13 as write protect (input)
  //  P0.20 as chip select (output), P0.12 as card present (input), and P1.24 as write protect (input)
  //
  GPIO0_FIOSET  = GPIO_IO_P20;
  GPIO0_FIODIR |= GPIO_IO_P20;
  //LUK_GPIO0_FIODIR &= ~(GPIO_IO_P13 | GPIO_IO_P12);
  // write protect: GPIO_IO_P13 --> GPIO_IO_P13 --> P1.24
  // card present:  GPIO_IO_P12 --> GPIO_IO_P12 --> P1.25
  GPIO1_FIODIR &= ~(WRPROTECT | CARDPRESENT);
