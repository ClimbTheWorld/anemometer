...
#define testAllFunctions
#define testWithoutADC

/** max. time between flowcount impulses that they are valid to count. If the time is longer, it must be that the pump just started. */
#define flowCountMaxDeltaTimeInSec   40
...
#define logAllTimeSummary
  ...
  /*! available Ports WITH special functions available and how are they used at Olimex's LPC-P2148 Evaluation Board.
      ...
      -P0.15     RI(UART1) EINT2*      AD1.5        Button 1: change to next screen (LCD)
      -P0.16     EINT0*    Match0.2T0  Capture0.2T0 Button 2: change to last screen (LCD)
  ...
  */
  ...
  #define WsPriceRappenStandardValue 0.2f
